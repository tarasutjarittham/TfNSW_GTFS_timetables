...
class _Base(object):
    ...
    @classmethod
    def load(cls, db, **kwargs):    #Tara: we need to add arg modified_date and transport mode here (gtfs.py will parse this)
        """Load method for ORM
        ...
        if cls.datasource == config.DATASOURCE_GTFS:
            directory = kwargs.get('gtfs_directory')    #Tara: We might have to append date columns to directory var
        elif cls.datasource == config.DATASOURCE_LOOKUP:
            directory = resource_filename('gtfsdb', 'data') #This is the data directory in gtfs folder

        records = []
        file_path = os.path.join(directory, cls.filename)   #Get the full path to txt file
        if os.path.exists(file_path):
            f = open(file_path, 'r')
            utf8_file = util.UTF8Recoder(f, 'utf-8-sig')
            reader = csv.DictReader(utf8_file)  #Tara: We might have to append date columns to reader var

            ###Add field vehicle position and modified_date here
            ...
            reader.fieldnames = [field.strip().lower() for field in reader.fieldnames] #Redundant, reader already has fieldnames attribute
            table = cls.__table__
            #try:
            ...
            i = 0
            for row in reader:  #Tara: We might have to append date columns to reader var

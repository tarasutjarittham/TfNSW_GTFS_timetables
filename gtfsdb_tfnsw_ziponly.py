...
def checkLastDate(response_head, url):
    ...
    if checkFileStatus(url) == 'file exists - key exists':
        ...
        if latest_modified > last_mod_date:
            ...
            logger.info('%s - Response has been updated. Updating json file...', opts.mode)
            ...
            logger.info('%s - modified_date has not been updated since last download', opts.mode)
    ...
    # File does not exist
        ...
        try:
            ...
            logger.error('%s - Error writing json object - %s', opts.mode, e)
        return {'keep_running':True, 'date_modified':latest_modified}
...
def saveZip(response, fileName, mod_date):
    ...
    try:
        logger.info('%s - saving zip file', opts.mode)
        ...
        logger.info('%s - Error: Could not save file', opts.mode)
    ...
    try:
        ...
        if keep_running:
            while keep_running:    # If true
                ...
                logger.info('%s - Downloading response......please wait.......', opts.mode)
                response = requests.get(opts.url, headers=headers)

                if checkStatus(response) == 200:
                    print("status check........200 OK")
                    #logger.info('%s - status 200 save response to zip file', opts.mode)
                    ...
                    logger.warning('%s - status %s Attempt to download again', opts.mode, checkStatus(response))

        else:
            print("no file is downloaded since nth new is updated")
            #logger.info('%s - no file is downloaded since nth new is updated', opts.mode)


    except ValueError, error:
        #print("Error: Sth went wrong")
        logger.info('%s - Error: Sth went wrong - %s', opts.mode, error)
